...
function EnemyBehavior.SetupScenarioDespawnHook()
    ...
    sdk.hook(method, function(args)
        ...
        -- Never block scenario standby teardown during scene/character loads.
        -- Skipping this freezes the Jill→Carlos RPD handoff on a black screen
        -- (or endless NOW LOADING).
        if Scene ~= nil and Scene.isTransitioning and Scene.isTransitioning() then
            return
        end
        if Scene ~= nil and Scene.isLoadingGameScene and Scene.isLoadingGameScene() then
            return
        end
