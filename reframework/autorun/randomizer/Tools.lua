...
function Tools.ShowGUI()
    ...
    local enemy_speed_text = '   (not connected)'
    local enemy_speed_color = AP_REF.HexToImguiColor('FFFFFF')
    local enemy_invisible_text = '   (not connected)'
    local enemy_invisible_color = AP_REF.HexToImguiColor('FFFFFF')
    local enemy_kills_text = '   (not connected)'
    local enemy_kills_color = AP_REF.HexToImguiColor('FFFFFF')
    local files_text = '   (not connected)'
    local files_color = AP_REF.HexToImguiColor('FFFFFF')
    ...
    if Lookups.difficulty then
        ...
        if Archipelago.IsDeathLinkActive and Archipelago.IsDeathLinkActive() then
            deathlink_text = "On"
        elseif Archipelago.death_link then
            deathlink_text = "Locked"
        ...
        if Archipelago.enemy_movement_speed_mode and Archipelago.enemy_movement_speed_mode ~= "Off" then
            enemy_speed_text = "   " .. tostring(Archipelago.enemy_movement_speed) .. " (" .. tostring(Archipelago.enemy_movement_speed_mode) .. ")"
            enemy_speed_color = AP_REF.HexToImguiColor('FFFFFF')
        else
            enemy_speed_text = "Off"
            enemy_speed_color = AP_REF.HexToImguiColor('777777')
        end

        if Archipelago.invisible_enemy_mode and Archipelago.invisible_enemy_mode ~= "Off" then
            enemy_invisible_text = "   " .. tostring(Archipelago.invisible_enemy_mode)
            enemy_invisible_color = AP_REF.HexToImguiColor('FFFFFF')
        else
            enemy_invisible_text = "Off"
            enemy_invisible_color = AP_REF.HexToImguiColor('777777')
        end
        ...
        if Archipelago.files_as_locations then
            files_text = "   All"
            files_color = AP_REF.HexToImguiColor('FFFFFF')
        else
            files_text = "Off"
            files_color = AP_REF.HexToImguiColor('777777')
        end
    ...
    imgui.set_next_window_size(Vector2f.new(320, 690), 0)
    ...
    imgui.text_colored(" Enemy Speed: ", -10825765)
    imgui.same_line()
    imgui.text_colored("                 " .. enemy_speed_text, enemy_speed_color)

    imgui.text_colored(" Invisible Enemies: ", -10825765)
    imgui.same_line()
    imgui.text_colored("        " .. enemy_invisible_text, enemy_invisible_color)

    imgui.text_colored(" Enemy Kills: ", -10825765)
    imgui.same_line()
    imgui.text_colored("                   " .. enemy_kills_text, enemy_kills_color)

    imgui.text_colored(" Files: ", -10825765)
    imgui.same_line()
    imgui.text_colored("                            " .. files_text, files_color)
    imgui.new_line()
    ...
    if Lookups.difficulty then
        imgui.new_line()
        imgui.text_colored(" Item Tracking:", AP_REF.HexToImguiColor('09ba39'))
        if ItemIndicator and ItemIndicator.DrawSettings then
            ItemIndicator.DrawSettings()
        end
