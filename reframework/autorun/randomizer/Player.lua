...
-- Jill only. Carlos has no parasite state, so bail before touching anything.
...
-- Jill only, same deal. This is the parasite cure vomit animation.
function Player.Puke()
    ...
    return pcall(function()
        sc:set_field("_ReserveResistParasite", true)
        sc:set_field("_IsParasite", true)
    end)
end
