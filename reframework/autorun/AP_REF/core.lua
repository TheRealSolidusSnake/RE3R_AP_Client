...
local function safe_callback(callback, ...)
	...
	if not ok then
		log.error("[AP_REF] callback error: " .. tostring(err))
	end
end
...
function APConnect(host)
	...
	set_socket_connected_handler(function()
		safe_callback(AP_REF.on_socket_connected)
	end)
	set_socket_error_handler(function(msg)
		safe_callback(AP_REF.on_socket_error, msg)
	end)
	set_socket_disconnected_handler(function()
		safe_callback(AP_REF.on_socket_disconnected)
	end)
	set_room_info_handler(function()
		safe_callback(AP_REF.on_room_info)
	end)
	set_slot_connected_handler(function(slot_data)
		safe_callback(AP_REF.on_slot_connected, slot_data)
	end)
	set_slot_refused_handler(function(reasons)
		safe_callback(AP_REF.on_slot_refused, reasons)
	end)
	set_items_received_handler(function(items)
		safe_callback(AP_REF.on_items_received, items)
	end)
	set_location_info_handler(function(items)
		safe_callback(AP_REF.on_location_info, items)
	end)
	set_location_checked_handler(function(locations)
		safe_callback(AP_REF.on_location_checked, locations)
	end)
	set_data_package_changed_handler(function(data_package)
		safe_callback(AP_REF.on_data_package_changed, data_package)
	end)
	set_print_handler(function(msg)
		safe_callback(AP_REF.on_print, msg)
	end)
	set_print_json_handler(function(msg, extra)
		safe_callback(AP_REF.on_print_json, msg, extra)
	end)
	set_bounced_handler(function(bounce)
		safe_callback(AP_REF.on_bounced, bounce)
	end)
	set_retrieved_handler(function(map, keys, extra)
		safe_callback(AP_REF.on_retrieved, map, keys, extra)
	end)
	set_set_reply_handler(function(message)
		safe_callback(AP_REF.on_set_reply, message)
	end)
end
...
re.on_pre_application_entry("UpdateBehavior", function() 
	...
	if disconnect_client then
		if AP_REF.APClient ~= nil and AP_REF.on_socket_disconnected ~= nil then
			safe_callback(AP_REF.on_socket_disconnected)
		end
		AP_REF.APClient = nil
		collectgarbage("collect")
